...
export default function Lobby({ gameCode, setGameCode }) {
  ...
  const onGameStart = () => {
    ...
    navigate(`/game?mode=multiplayer&gameCode=${gameCode}`);
  };
  useEffect(() => {
    if (connectionPending) {
      // Kolla om vi har en uppkoppling till servern 2 gånger per sekund.
      setInterval(checkIsConnected, 500);
    } else if (errorMessage === null) {
      // Om vi har en uppkoppling till servern, gå vidare
      ...
      if (gameCode === "new") {
        ...
        socket.on("createGame", (response) => {
          // Lyssna efter information om nya spel
          ...
          if (response.status === "success") {
            ...
            // Hantera fel
            console.warn(
              `Misslyckades med att skapa ett nytt spel: ${response.errorMessage}.`
            );
        ...
        if (gameInformation === null) {
          ...
          socket.on("gameInfo", (response) => {
            ...
            if (response.status === "success") {
              ...
              console.warn(
                `Misslyckades med att inhämta spelinformation: ${response.errorMessage}.`
              );
              ...
              if (
                ...
                response.errorType === "gameNotFound"
              ) {
                navigate(`?gameCode=${gameCode}&error=gameNotFound`);
              } else {
                // Om vi får ett generiskt fel.
                setErrorMessage(response.errorMessage);
              }
            }
          });
          ...
          socket.on("joinGame", (response) => {
            ...
            if (response.status === "success") {
              ...
              socket.on("gameUpdate", (response) => {
                console.log(
                  "Tog emot information om uppdaterad speldata: ",
                  response
                );
                if (response.status === "success") {
  ...
  if (errorMessage !== null) {
    ...
    return (
      <LoadingSpinner fullPage={true} text="Ansluter till spelservern..." />
    );
    ...
    if (!meFound) {
      // Läs ovan: ett litet hack som jag applicerar för att rendera saker korrekt
      playerElements.splice(
        0,
        0,
        <Player name={player.name} score={null} type="card" isMe={true} />
      ); // Lägg till information om den aktuella spelaren först i listan.
    }
    // Kolla om spelet kan startas eller inte.
    const gameCanNotBeStarted =
      connectionPending ||
      gameInformation.players === undefined ||
      (gameInformation.players.length < 2 &&
        !(!meFound && gameInformation.players.length === 1));
    let startGameElements = [
      <Button
        text={"Starta spel"}
        disabled={gameCanNotBeStarted}
        color="green"
        onClick={() => {
          ...
          socket.on("startGame", (response) => {
            ...
            if (response.status === "success") {
              ...
              console.warn(
                "Det verkar som att spelet misslyckades med att starta!"
              );
              setErrorMessage(
                `Kunde inte starta spelet på grund av följande fel: ${response.message}`
              );
            }
          });
    ...
    if (gameCanNotBeStarted) {
      // Om spelet inte kan startas, lägg till en instruktion varför under knappen.
      startGameElements.push(
        <p className="text-sm text-white py-3">
          Spelet kan startas när minst 1 spelare (förutom du) har anslutit.
        </p>
      );
    }
    startGameElements.push(
