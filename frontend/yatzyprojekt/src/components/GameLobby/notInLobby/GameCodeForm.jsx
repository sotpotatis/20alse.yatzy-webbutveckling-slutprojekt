...
export default function GameCodeForm({ gameCode, onGameCode }) {
  ...
  const validGameCodeEnterred =
    !Number.isNaN(Number.parseInt(gameCode)) && Number(gameCode) > 0;
  ...
  if (searchParams.get("error") === "gameNotFound" && !gameCodeHasBeenUpdated) {
    inputFieldInvalidExplanation =
      "Hittade inget spel med den koden. Kolla med den som skickat koden till dig!";
  ...
  const onJoinRequest = () => {
    // Funktion för när spelare vill gå med
  ...
  return (
    <div key="game-code-form">
      <form
        ...
        onSubmit={(event) => {
          // Gå med i spelet när man klickar på enter
      ...
      >
        <InputField
          type={"text"}
          value={gameCode}
          invalid={showInputFieldInvalidMessage}
          invalidExplanation={inputFieldInvalidExplanation}
          placeholder={"Kod"}
          label={"Spelkod"}
          id="game-code-input"
          onChange={(event) => {
        ...
        <Button
          type="button"
          disabled={!validGameCodeEnterred}
          text="Gå med"
          title={
            validGameCodeEnterred
              ? null
