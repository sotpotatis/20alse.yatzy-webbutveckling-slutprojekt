...
export default function GameModePrompt(props) {
  ...
  const openGame = () => {
    ...
    if (userChoices.gameType === "multiplayer") {
      ...
      navigate(
        `/game?mode=local&numberOfPlayers=${userChoices.numberOfPlayers}`
      ); // Omdirigera till själva spelplanen.
    }
  };
  let title = null;
  let children = null;
    ...
    if (userChoices.gameType === "multiplayer") {
      ...
      children = [
        <div className="grid grid-cols-3">
          <Button
            screenReaderText="Minska antalet spelare."
            color="lightBlue"
            circle={true}
          ...
          <Button
            screenReaderText="Öka antalet spelare."
            color="lightBlue"
            circle={true}
  ...
  return <Container title={title} children={children} />;
}
