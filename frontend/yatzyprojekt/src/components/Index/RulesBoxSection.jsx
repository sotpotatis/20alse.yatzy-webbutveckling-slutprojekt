...
export default function RulesBoxSection({ title, icon, children }) {
  ...
  }; // Skapa en funktion när elementet iterageras med
  let allElementsToRender = [
    <div key={`${title}-heading`} className="grid grid-cols-2">
      ...
      <Icon
        ...
        className="text-4xl text-gray-400 hover:text-gray-500 hover:cursor-pointer justify-self-end"
        onClick={onInteract}
        onKeyDown={(event) => {
          runOnEnterPress(event, onInteract);
        }}
        tabIndex="0"
      />
    </div>,
