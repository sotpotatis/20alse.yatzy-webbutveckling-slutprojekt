...
export default function ScoreBoard({
  ...
  let isPickingScore =
    player.name === gameState.currentPlayerName && gameState.isPickingScore;
  for (const scoreId of Object.keys(possibleDiceStates)) {
    ...
    let score = hasBeenClaimed
      ...
      ? tentativePoints !== null &&
        Object.keys(tentativePoints).includes(scoreId)
    ...
    scoreElements.push(
      <ScoreBoardScore
        title={title}
        points={score}
        pickMode={isPickingScore}
        onPick={
          !hasBeenClaimed && isPickingScore
            ? () => {
                onScorePick(scoreId);
              }
            : null
    ...
    <>
      <div key="scoreboard-heading" className="flex flex-row gap-x-4">
        <Heading level={1} size={2}>
          {gameState.currentPlayerName}'s poäng
        </Heading>
        <ScoreBoardBadge
          size="big"
          points={currentPlayerTotalPoints}
          isPickable={false}
        />
      </div>
      <div
        key="scoreboard-body"
        className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 xl:grid-flow-col grid-rows-5"
      >
        {scoreElements}
      </div>
    </>
