...
export default function Heading({
  ...
  let htmlTagToUse = null;
  // Specialfall: för level "text" så matchar storleken en titel men vi renderar som en p-tagg.
  // Detta är för att förbättra kompatibilitet med skärmläsare.
  if (level === "text") {
    htmlTagToUse = "p";
  } else {
    htmlTagToUse = `h${level === null ? size : level}`;
  }
  ...
  return React.createElement(
    // Skapa ett element som matchar med den storlek som vi har valt
    htmlTagToUse,
    {
      className:
        headingClasses +
        " " +
        commonClasses +
        (additionalClasses !== undefined ? ` ${additionalClasses}` : ""),
    },
    childrenElements
  );
}
