...
function validatePlayerAuthorization(models, secretKey, callback){
    ...
    models.Player.scope("withSecret").findAll({where: {secret: secretKey}, include: [{
        model: models.Score,
        as: "scores"
        }]}).then((data)=>{
 ...
 */
function waitForDiceUpdate(dices, updateFunction, callback, timeoutCallback=null){
...
function prepareForPlayer(game, models, socket, newPlayerName, callback){
    ...
    game.save().then(()=>{
        ...
        waitForDiceUpdate(game.dices, (dice)=>{
            dice.saved=false
        dice.number="empty"
        return dice}, (game, models, socket,)=>{
...
function updateRound(game, models, socket, onScorePick, whenPrepared=null){
    ...
    if (playerTimeout !== null){ // Rensa tidigare kontroller efter inaktiva användare.
    ...
    prepareForPlayer(game, models, socket, nextPlayer.name, ()=>{
        ...
        if (whenPrepared !== null){ // Kör extra callback om specificerad
            whenPrepared(game)
        }
        ...
        playerTimeout = setTimeout(()=>{
            if (game.currentPlayerName === initialPlayerName){
                console.log("Timeout uppnåddes: en spelare har inte gjort något på en minut. Byter spelare...")
...
function toggleDiceLocked(game, number, callback, timeoutCallback=false){
    ...
    waitForDiceUpdate([game.dices[number]], (dice)=>{
...
for (const player of game.players) {
    console.log(`Detektering av spelslut: spelarens poänglängd är: ${player.scores.length}`)
...
const runGame = ((game, models, socket, whenPrepared=null) => { // Skapa en rekursiv funktion som kör spelet.
    ...
    if (detectGameEnd(game)) {
        ...
        updateRound(game, models, socket,  () => {
...
const includeEverything = (models) => { // Inkluderar all information som har med ett spel att göra
   return [ {
        model: models.Player,
        as: "players",
        include: {
...
const socketHandlers = {
    ...
    "rollDice": (message, socket, models)=>{
        ...
        if (message.gameCode === undefined){
            socket.emit("rollDice", generateErrorResponse("Du har inte inkluderat en spelkod i ditt meddelande"))
        }
        else {
            findGameById(models, message.gameCode.toString(), includeEverything(models), (game)=>{
                ...
                generateDiceNumbers(game, models, socket,()=>{
                        ...
                        socket.emit("rollDice", generateSuccessResponse({message: "Tärningarna har kastats om.", gameState: game}))
                        sendGameStateUpdate(models, game.gameCode, socket)
                },
                ()=>{
                    socket.emit("rollDice", generateErrorResponse("Timeout när databasen skulle kontaktas. Vänligen försök att ansluta igen lite senare."))
                })
                }
                else {
                    socket.emit("rollDice", generateErrorResponse("Ett oväntat fel inträffade: spelet verkar inte längre existera. Testa att starta ett nytt spel."))
                }
    ...
    "toggleDice": (message, socket, models)=>{
        ...
        if (message.gameCode === undefined){
            socket.emit("toggleDice", generateErrorResponse("Du har inte inkluderat en spelkod i ditt meddelande"))
        }
        else if (message.diceIndex ===  undefined || message.diceIndex < 0 || message.diceIndex > 4){
            socket.emit("toggleDice", generateErrorResponse("Du har angivit ett ogiltigt tärningsnummer i ditt meddelande."))
        }
        else {
            findGameById(models, message.gameCode.toString(), includeEverything(models), (game)=>{
                if (game !== null){
                // Hitta spel från meddelande
                toggleDiceLocked(game, message.diceIndex,()=>{
                        console.log("Tärningen har uppdaterats.")
                        socket.emit("toggleDice", generateSuccessResponse({message: "Tärningen har uppdaterats.", gameState: game}))
                        sendGameStateUpdate(models, game.gameCode, socket)
                },
                ()=>{
                    socket.emit("toggleDice", generateErrorResponse("Timeout när databasen skulle kontaktas. Vänligen försök att ansluta igen lite senare."))
                })}
                else {
                    socket.emit("toggleDice", generateErrorResponse("Ett oväntat fel inträffade: spelet verkar inte längre existera. Testa att starta ett nytt spel."))
                }
            })

        }
        },
    "possibleScores": (message, socket, models)=>{ // Lyssna efter förfrågan att hämta möjlig poäng
        console.log(`Tog emot en förfrågan att hämta möjliga poäng med spelkoden ${message.gameCode}`)
        if (message.gameCode === undefined){
            socket.emit("possibleScores", generateErrorResponse("Du har inte inkluderat en spelkod i ditt meddelande"))
        }
        else {
            findGameById(models, message.gameCode.toString(), includeEverything(models), (game)=>{
                if (game !== null){
                    ...
                    socket.emit("possibleScores", generateErrorResponse("Ett internt serverfel inträffade när spelet skulle uppdateras. Vänligen försök igen lite senare."))
                    sendGameStateUpdate(models, game.gameCode, socket)
                    })
                }
                else {
                    socket.emit("possibleScores", generateErrorResponse("Ett oväntat fel inträffade: spelet verkar inte längre existera. Testa att starta ett nytt spel."))
                }
            })}
    },
    // Lyssna efter förfrågan att skriva in sin poäng
      "pickScore": (message, socket, models)=> {
          console.log("Tog emot en förfrågan om att skriva in poäng.")
           if (message.gameCode === undefined){
            socket.emit("pickScore", generateErrorResponse("Du har inte inkluderat en spelkod i ditt meddelande"))
        }
           else {
                           findGameById(models, message.gameCode.toString(), includeEverything(models), (game)=>{
                               if (game !== null){
          ...
          updateUserScore(game, models, socket, message.requestedScoreType, (game) => {
              sendGameStateUpdate(models, game.gameCode.toString(), socket)
              runGame(game, models, socket, (game)=>{
                            socket.emit("pickScore", {message: "Poängen har uppdaterats.", gameState: game})
          })
          })
               }
                               else {
                    socket.emit("pickScore", generateErrorResponse("Ett oväntat fel inträffade: spelet verkar inte längre existera. Testa att starta ett nytt spel."))
                }})
           }
      },
                    ...
                    if (!validAuthentication){
                        ...
                        let scoreRemoveCheckInterval = setInterval(()=>{
                            if (removedScores === player.scores.length){
                                console.log("Alla tidigare poäng borttagna för spelaren.")
                                addPlayer(player) // Lägg till spelaren i spelet.
                                clearInterval(scoreRemoveCheckInterval)
                            }
                            else if (scoreRemoveCheckInterval >= 20){
                                console.warn("Timeout när tidigare poäng för användaren skulle tas bort! Skickar meddelande...")
                                socket.emit("joinGame", generateErrorResponse("Timeout vid autentisering när dina tidigare poäng skulle rensas. Vänligen försök att gå med i spelet lite senare."))
                                clearInterval(scoreRemoveCheckInterval)
                            }
