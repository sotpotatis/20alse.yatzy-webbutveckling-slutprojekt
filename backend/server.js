...
const SOCKET_SERVER_PORT =
  parseInt(process.env.SOCKET_SERVER_PORT) ||
  parseInt(process.env.PORT) ||
  3000; // Standardport är 3000
...
assert.ok(
  DATABASE_USER_NAME,
  "Du har inte specificerat en användarnamn för databasen."
);
assert.ok(
  DATABASE_USER_PASSWORD,
  "Du har inte specificerat ett lösenord för databasen."
);
...
console.log(
  `Kommer att koppla upp till ${DATABASE_SERVER}:${DATABASE_PORT} som en användare med lösenord ${DATABASE_USER_NAME} och använda databasen ${DATABASE_NAME}.`
);
...
sequelize
  ...
  .catch((error) => {
    // Ifall ett fel uppstår vid uppkoppling till databasen
...
sequelize
  ...
  .then(() => {
    ...
    const socketServer = new Server(httpServer, {
      cors: {
        // Specificera CORS-regler, annars kommer inte klienter i webbläsare vela koppla upp sig.
        origin: "*",
        methods: ["GET", "POST"],
      },
    });
  ...
  .catch((error) => {
    // Ifall fel inträffar under synkroniseringen eller under applikationen
