...
function randomNumberExcept(start, end, exclusions) {
  if (!Array.isArray(exclusions)) {
    // Konvertera enskilda argument till en lista.
    exclusions = [exclusions];
  }
  while (true) {
    // Tills ett nummer som inte är exkluderat genererats
    const generatedNumber = randomNumber(start, end);
    if (!exclusions.includes(generatedNumber)) {
      // Om numret som genereras inte ska exkluderas, returnera det
